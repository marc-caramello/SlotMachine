...
public class Row : MonoBehaviour
{
    ...
    private IEnumerator Rotate()
    {
        ...
        for (int k = 0; k < 8; k++)
        {
            if (k == 1)
            {
                if (FloatsAreEqual(transform.position.y, -1.65f))
                {
                    stoppedSlot = k;
                }
            }
            if (FloatsAreEqual(transform.position.y, start + (k * 1f)))
            {
                stoppedSlot = k;
    ...
    private bool FloatsAreEqual(float f1, float f2)
    {
        float roundedA = Mathf.RoundToInt(f1);
        float roundedB = Mathf.RoundToInt(f2);
